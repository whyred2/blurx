...
const MovieComments = ({ movieId }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
          try {
            const response = await axios.get(`http://localhost:3001/comment/movie/${mediaId}`);
    ...
    const handleCommentLike = async (commentId) => {
        ...
        if (commentId.comment_id) {
            endpoint = `comment/movie/${commentId.id}/like-reply`;
            comment_id = commentId.id;
            console.log('like-reply')
        } else {
            endpoint = `comment/movie/${commentId}/like`;
    ...
    const handleCommentDislike = async (commentId) => {
        ...
        let endpoint = '';
        if (commentId.comment_id) {
            endpoint = `comment/movie/${commentId.id}/dislike-reply`;
            console.log('like-reply')
        } else {
            endpoint = `comment/movie/${commentId}/dislike`;
            console.log('like')
        }
    ...
    const handleCommentDelete = async (commentId) => {
        ...
        let endpoint = '';
        if (commentId.comment_id) {
            endpoint = `comment/movie/${commentId.id}/del-comment-reply`;
            console.log('del-comment-reply')
        } else {
            endpoint = `comment/movie/${commentId}/del-comment`;
    ...
    const handleComplaintSubmit = async (commentId) => {
        try {
            ...
            console.log('SERIES_ID', movieId);
            console.log('COMMENT_ID', selectedCommentId);
            console.log('TEXT', complaintText);

            let endpoint = '';
            if (commentId.comment_id) {
                endpoint = `comment/movie/${commentId.id}/complaint-reply`;
        ...
        <div className='comments-container content-conteiner'>
            {authenticated ? (
                <>
                    ...
                    <AddComment mediaId={mediaId} type='movie'/>
                        ...
                        {comments.map((comment, index) => (
                            <div key={index} className='comment-item'>
                                ...
                                {showReplyForm && selectedCommentId === comment.id && (
                                    <AddComment mediaId={mediaId} type='movie' commentId={comment.id} reply={true} onCancelReply={handleCancelReply} />
                                )}
                            </div>
                        ))}
                    </div>
