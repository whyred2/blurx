...
const Filters = ({ setFilteredContent }) => {
    const [genres, setGenres] = useState([]);
    const [minYear, setMinYear] = useState(1959); // Минимальный год
    const [maxYear, setMaxYear] = useState(2024); // Максимальный год
    ...
    return (
        <div className='filter-conteiner content-main'>
            <div className='content-header-title'>
                <div className='sort'>
                <div className='content-icon'>
                    <LuMenu size={32} min={32}/>
                </div>
            ...
            <div className='filter-group year'>
                <div className='row'>
                    <InputRange
                        minValue={1959} // Минимальное значение даты
                        maxValue={2024} // Максимальное значение даты
                        value={{ min: minYear, max: maxYear }} // Текущее значение диапазона дат
                        onChange={value => {
                            setMinYear(value.min);
                            setMaxYear(value.max);
                        }} // Обработчик изменения диапазона дат
                        formatLabel={(value) => value} // Формат для отображения значений
                        step={1}
                    />
                        <div className='marker-range'>
                            ...
                            <div className='verical-line-100' style={{left: 50.7692 + '%'}}>                                
                            </div>                            
                            <>
